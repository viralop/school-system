...
<head>
    ...
    <title>{{ __('messages.Setup') }} - خولة بنت الأزور</title>
...
<body class="min-h-screen flex items-center justify-center p-4">
    ...
    <div class="glass rounded-2xl shadow-2xl w-full max-w-md p-8 animate-scale-in">
        ...
        <div class="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-6">
            <p class="text-blue-400 text-sm text-center">{{ __('messages.No supervisor account exists yet. Create the first administrator account to get started.') }}</p>
        </div>

        <form method="POST" action="{{ route('setup.store') }}">
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-[var(--text-label)] text-sm font-medium mb-2">{{ __('messages.Full Name') }}</label>
                <div class="relative">
            ...
            <div class="mb-4">
                <label for="email" class="block text-[var(--text-label)] text-sm font-medium mb-2">{{ __('messages.Email Address') }}</label>
                <div class="relative">
                    <div class="absolute inset-y-0 {{ $isRtl ? 'right-0 pr-3' : 'left-0 pl-3' }} flex items-center pointer-events-none">
            ...
            <div class="mb-4">
                <label for="password" class="block text-[var(--text-label)] text-sm font-medium mb-2">{{ __('messages.Password') }}</label>
                <div class="relative">
                    <div class="absolute inset-y-0 {{ $isRtl ? 'right-0 pr-3' : 'left-0 pl-3' }} flex items-center pointer-events-none">
            ...
            <div class="mb-6">
                <label for="password_confirmation" class="block text-[var(--text-label)] text-sm font-medium mb-2">{{ __('messages.Confirm Password') }}</label>
                <div class="relative">
                    <div class="absolute inset-y-0 {{ $isRtl ? 'right-0 pr-3' : 'left-0 pl-3' }} flex items-center pointer-events-none">
