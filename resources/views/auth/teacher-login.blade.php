...
<head>
    ...
    <title>@lang('messages.Teacher Login') - خولة بنت الأزور</title>
...
<body class="min-h-screen flex items-center justify-center p-4">
    ...
    <div class="glass rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div class="text-center mb-8">
            ...
            <p class="text-[var(--text-muted)] mt-1 text-sm">@lang('messages.Teacher Login')</p>
        </div>

        <form method="POST" action="{{ route('teacher.login.step1') }}">
            @csrf
            <div class="mb-4">
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}"
                        class="w-full bg-[var(--bg-input)] text-[var(--text-primary)] border border-[var(--border-input)] rounded-xl {{ $isRtl ? 'pr-10 pl-4' : 'pl-10 pr-4' }} py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40 placeholder-[var(--text-placeholder)]" required autofocus>
                </div>
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    ...
                    <input type="password" name="password" id="password"
                        class="w-full bg-[var(--bg-input)] text-[var(--text-primary)] border border-[var(--border-input)] rounded-xl {{ $isRtl ? 'pr-10 pl-4' : 'pl-10 pr-4' }} py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40 placeholder-[var(--text-placeholder)]" required>
                </div>
            </div>
        ...
        <div class="text-center mt-6 space-y-2">
            <a href="{{ route('password.request') }}" class="text-amber-400/60 hover:text-amber-300 text-sm block transition">@lang('messages.Forgot Password?')</a>
            <a href="{{ route('teacher.signup') }}" class="text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-sm transition">@lang('messages.Don\'t have an account? Register')</a>
        </div>
