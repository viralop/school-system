...
<head>
    ...
    <title>@lang('messages.Student Dashboard') - خولة بنت الأزور</title>
...
<body class="min-h-screen">
    ...
    <div class="max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        @foreach($monthlyGrades as $examId => $examGrades)
            @php
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="text-[var(--text-secondary)] text-sm border-b border-[var(--border-main)]">
                                ...
                                <th class="text-center pb-2">@lang('messages.Degree')</th>
                                <th class="text-center pb-2">@lang('messages.Max Degree')</th>
                        ...
                        <tbody>
                            @foreach($examGrades as $grade)
                                @php
                                    $maxDeg = $grade->subject->default_max_degree;
                                @endphp
                                <tr class="border-b border-[var(--border-main)]" style="--tw-border-opacity: 0.3">
                                    ...
                                    <td class="py-2 text-[var(--text-secondary)] text-sm text-center">{{ $maxDeg }}</td>
                                    <td class="py-2 text-[var(--text-primary)] text-sm text-center font-medium">
                                        {{ round(($grade->score / $maxDeg) * 100, 1) }}%
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="bg-[var(--bg-card)] rounded-xl border border-[var(--border-main)] p-6 mb-6 shadow-[var(--shadow-card)]">
                ...
                <div class="mobile-scroll-table overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            @foreach($termGradesList as $grade)
                                <tr class="border-b border-[var(--border-main)]" style="--tw-border-opacity: 0.3">
                                    <td class="py-2 text-[var(--text-primary)] text-sm">{{ $grade->subject->localizedName }}</td>
                                    <td class="py-2 text-[var(--text-primary)] text-sm text-center">{{ $grade->score }}</td>
                                    <td class="py-2 text-[var(--text-secondary)] text-sm text-center">{{ $grade->subject->default_max_degree }}</td>
                                    <td class="py-2 text-[var(--text-primary)] text-sm text-center font-medium">
                                        {{ round(($grade->score / $grade->subject->default_max_degree) * 100, 1) }}%
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        @if($termGrades->isEmpty() && $monthlyGrades->isEmpty())
