    ...
    <div class="max-w-7xl mx-auto px-4 py-8 md:py-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            ...
            <div class="bg-white/10 backdrop-blur-md p-6 md:p-8 rounded-xl border border-white/20 text-center">
                ...
                <h3 class="text-white font-semibold text-lg mb-2">@lang('messages.Secure')</h3>
                <p class="text-green-300/80 text-sm">@lang('messages.Secure system for students, teachers, and supervisors')</p>
            </div>
        </div>
        ...
        @if($achievements->count() > 0)
            <div class="mb-16">
                <h2 class="text-2xl font-bold text-white mb-6">@lang('messages.Latest Achievements')</h2>
                    ...
                    @foreach($achievements as $achievement)
                        <div class="bg-white/10 backdrop-blur-md p-6 rounded-xl border border-white/20">
                            ...
                            @if($achievement->description($locale))
                                <p class="text-green-300/80 text-sm mt-2">{{ $achievement->description($locale) }}</p>
                            @endif
                        </div>
                    @endforeach
                </div>
                <a href="{{ route('achievements') }}" class="inline-block mt-4 text-blue-400 hover:text-blue-300 text-sm">@lang('messages.View all') &rarr;</a>
            </div>
        @endif
    </div>
@endsection
