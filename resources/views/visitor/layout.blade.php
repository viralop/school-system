...
<body class="min-h-screen flex flex-col">
    <nav class="bg-[var(--bg-card-90)] border-b border-[var(--border-main)] backdrop-blur-xl sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            <div class="flex items-center gap-8">
                <a href="{{ route('home') }}" class="flex items-center gap-2.5">
                    ...
                    <span class="text-xl font-bold text-[var(--text-primary)]">{{ $schoolName }}</span>
                </a>
                <div class="flex items-center gap-5">
                    <a href="{{ route('home') }}" class="nav-link text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-sm transition @yield('nav-home-active')">@lang('messages.Home')</a>
                    <a href="{{ route('about') }}" class="nav-link text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-sm transition @yield('nav-about-active')">@lang('messages.About')</a>
                    <a href="{{ route('achievements') }}" class="nav-link text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-sm transition @yield('nav-achievements-active')">@lang('messages.Achievements')</a>
                </div>
            </div>
    ...
    <footer class="bg-[var(--bg-footer)] border-t border-[var(--border-main)] py-8 mt-12">
        <div class="max-w-7xl mx-auto px-4 text-center">
            <div class="flex items-center justify-center gap-2 mb-3">
                <div class="w-6 h-6 rounded-lg bg-blue-600/20 flex items-center justify-center">
                    <svg class="w-3.5 h-3.5 text-blue-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5"/></svg>
                </div>
                <span class="text-[var(--text-primary)] font-semibold">{{ $schoolName }}</span>
            </div>
            <p class="text-[var(--text-tertiary)] text-sm">&copy; {{ date('Y') }} {{ $schoolName }}. {{ $locale === 'ar' ? 'جميع الحقوق محفوظة' : 'All rights reserved' }}.</p>
        </div>
    </footer>
</body>
</html>
